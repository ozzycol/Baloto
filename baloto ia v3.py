...
if resultados:
    ...
    if len(pred_rf_prox_sorteo) == 1:
        # Si solo se predijo una única salida, asumimos que todas las balotas y la superbalota tienen el mismo valor
        numeros_balotas_rf = [int(round(pred_rf_prox_sorteo[0]))] * 5
        superbalota_rf = int(round(pred_rf_prox_sorteo[0]))
    else:
        numeros_balotas_rf = sorted([int(round(pred_rf_prox_sorteo[i])) for i in range(5)])
        superbalota_rf = int(round(pred_rf_prox_sorteo[5]))

    if len(pred_gb_prox_sorteo) == 1:
        numeros_balotas_gb = [int(round(pred_gb_prox_sorteo[0]))] * 5
        superbalota_gb = int(round(pred_gb_prox_sorteo[0]))
    else:
        numeros_balotas_gb = sorted([int(round(pred_gb_prox_sorteo[i])) for i in range(5)])
    ...
    # Simulación de 20000000 juegos para encontrar la combinación con mayor probabilidad de ganar
    def generar_combinacion():
        return tuple(random.sample(range(1, 44), 5)), random.randint(1, 16)

    resultados_simulacion = Counter()

    for _ in range(20000000):
        combinacion = generar_combinacion()
        resultados_simulacion[combinacion] += 1

    mejor_combinacion, veces_ganadas = resultados_simulacion.most_common(1)[0]

    print("La combinación con mayor probabilidad de ganar es:", mejor_combinacion)
    print("Se ha ganado", veces_ganadas, "veces.")
else:
    print("No se pudieron obtener los resultados de la base de datos.")
