...
query = "SELECT balota1, balota2, balota3, balota4, balota5, superbalota, FECHA FROM baloto WHERE FECHA >= %s ORDER BY FECHA DESC"
cursor.execute(query, (fecha_inicio,))
resultados = cursor.fetchall()

# Obtener las balotas y superbalotas de las últimas cuatro jugadas
ultimas_jugadas = resultados[:4]
...
# Crear una lista plana de todas las balotas y superbalotas
todas_balotas = [balota for resultado in resultados for balota in resultado[:5]]
todas_superbalotas = [resultado[5] for resultado in resultados]
